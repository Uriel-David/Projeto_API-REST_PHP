<?php
  ...
  class AthletesTenis
  {
    ...
    public static function insertAthlete($data)
    {
        ...
        ' (`id`, `nome`, `nif`, `morada`, `telefone`, `email`, `data_nascimento`, `altura`, `peso`, `modalidade`, `mao_preferencial`, `ranking_mundial`) VALUES
    ...
    public static function updateAthlete($id, $data)
    {
      ...
      $sql = 'UPDATE '.self::$table.' SET nome = :nome, nif = :nif, morada = :morada, telefone = :telefone, email = :email, data_nascimento = :nascimento, altura = :altura, peso = :peso, modalidade = :modalidade, mao_preferencial = :mao_preferencial, ranking_mundial = :ranking_mundial WHERE '.self::$table.'.id = '.$id;
      $stmt = $connectionPDO->prepare($sql);
      ...
      if($data['mao_preferencial'] == null || "") {
          $athleteData[0]["mao_preferencial"];
      } else {
          $stmt->bindValue(':mao_preferencial', $data['mao_preferencial']);
      }

      if($data['ranking_mundial'] == null || "") {
          $athleteData[0]["ranking_mundial"];
      } else {
          $stmt->bindValue(':ranking_mundial', $data['ranking_mundial']);
      }
